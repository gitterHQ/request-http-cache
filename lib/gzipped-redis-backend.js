...
GZippedRedisBackend.prototype = {
  ...
  getContent: function (key, callback) {
    this.redisClient.hmget(this.contentPrefix + key, CONTENT_HASH_KEY, URL_KEY, function(err, result) {
      if (err) return callback(err);
  ...
  _persist: function(url, key, statusCode, bodyCompressed, headers, expiry, etag, callback) {
    ...
    var multi = this.redisClient.multi();
    // Update the content
    multi.hmset(this.contentPrefix + key, EXPIRY_HASH_KEY, expiry, ETAG_HASH_KEY, etag, CONTENT_HASH_KEY, proto, URL_KEY, url);
    multi.expire(this.contentPrefix + key, this.ttl);
